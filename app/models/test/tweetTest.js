...
describe('#tweet', function () {
	...
	it('** stemmed words **', function () {
		...
		expect(tweet.getStemmedWords()).to.eql({'AKRBL' : 3, 'TJR' : 3 });
	});
	...
	it('** init tweet **', function () {
		...
		expect(tweet.getText()).equal('Un policier jugé pour cannibalisme en Allemagne http://t.co/Y55M9ZkpNj');
		...
		expect(tweet.getStemmedWords()).to.eql({"ALMN": 1, "JK": 1, "KNBL": 1, "PLS": 1});
